"""Tests for climate.py entity naming and the restore-last-known-state
behavior.
...
See test_sensor.py's module docstring for the restore tests' general
testing pattern - climate has no RestoreSensor equivalent, so LKThermostat
uses plain RestoreEntity and reads current_temperature/target_temperature
back from the restored State's attributes instead of a typed native_value.
"""
...
class TestThermostatRestore:
    async def test_restores_temperatures_when_live_fetch_failed_and_fresh(
        ...
        restored_entity_id = "climate.test_restore_thermostat"
        _seed_restore_cache(hass, restored_entity_id, 19.5, 21.0, dt_util.utcnow())
    ...
    async def test_ignores_a_stale_restored_value(self, hass, fake_manager):
        ...
        _seed_restore_cache(hass, restored_entity_id, 19.5, 21.0, stale)
    ...
    async def test_live_value_takes_priority_over_a_restored_one(
        ...
        restored_entity_id = "climate.test_restore_thermostat"
        _seed_restore_cache(hass, restored_entity_id, 1.1, 2.2, dt_util.utcnow())
